...
from src.entities.food_controller import FoodController
...
class GameScene(Scene):
    def __init__(self, level=1, score=0, snake=None):
        ...
        # FoodController creates and manages the food instance and adds it to `all_sprites`
        self.food_controller = FoodController(self.all_sprites)
        self.food = self.food_controller.food
    ...
    def update(self, dt):
        ...
        if self.snake_controller.check_food_collision(self.food):
            ...
            # delegate spawning to the controller
            self.food_controller.spawn_food()
